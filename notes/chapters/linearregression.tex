...
A concise reference is chapter 4 in \citeasnoun{rodriguez2007generalized}. A poisson regression model assumes Poisson noise. Two notable factors are: 

\begin{itemize}
\item The data is strictly positive.
\item The Poisson distribution does not have separate parameters for the mean and the variance, specifically $\mu = \sigma$. That means the assumption is the distribution of the data is heteroskedastic, where higher counts also fluctuate more widely.
\end{itemize}
